...
def _(SkyCoord, ascii, create_fits_image, fits, numpy, os, plt, u, wcs):
    ...
    fname = os.path.join("..", "A2744_Harris23_GCs", "2508_skynoise_grid.txt")
    table = ascii.read(
        fname, names=["RA", "DEC", "x (px)", "y (px)", "Sky", "log(sigsky)"]
    )
    ...
    out_fname = os.path.join("..", "A2744_Harris23_GCs", "2508_skynoise_grid.fits")
    ...
    cb = ax.imshow(_img.T, origin="lower", cmap="viridis")
    # add the colorbar
